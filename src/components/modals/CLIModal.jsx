    ...
    >
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        ...
        <div className="bg-gray-900 rounded-xl p-4 border border-green-500/30 text-center">
          ...
          <div className="text-xs text-green-500/70 font-bold uppercase tracking-wider">Success</div>
        </div>
        <div className="bg-gray-900 rounded-xl p-4 border border-blue-500/30 text-center">
          <div className="text-2xl font-bold text-blue-400">{successRate}%</div>
          <div className="text-xs text-blue-500/70 font-bold uppercase tracking-wider">Rate</div>
        </div>
      </div>
        ...
        <div className="space-y-4 mb-8">
          {modalData.register_description && (
            <div className="bg-gray-900/50 border border-gray-700 rounded-lg p-4">
              ...
              <p className="text-sm text-gray-300 leading-relaxed">{stripEmojis(modalData.register_description)}</p>
            </div>
          )}
          {modalData.register_justification && (
            <div className="bg-gray-900/50 border border-gray-700 rounded-lg p-4">
              ...
              <p className="text-sm text-gray-300 leading-relaxed">{stripEmojis(modalData.register_justification)}</p>
            </div>
          )}
        </div>
      )}
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between border-b border-gray-700 pb-2 mb-4">
          ...
          <span className="text-xs text-gray-400 bg-gray-800 px-2 py-1 rounded-full border border-gray-700">{commandsList.length} entries</span>
        </div>
        ...
        {commandsList.length > 0 ? (
          commandsList.map((cmd, index) => {
            ...
            return (
              <div key={index} className="border border-gray-700 rounded-lg overflow-hidden bg-gray-800 transition-all hover:border-gray-600">
                <button
                ...
                >
                  <div className="flex items-center gap-3 overflow-hidden">
                    <div className={`flex-shrink-0 w-6 h-6 rounded flex items-center justify-center ${isSuccess ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
                      {isSuccess ? <CheckCircle size={14} /> : <XCircle size={14} />}
                    </div>
                    <div className="min-w-0">
                      <div className="text-sm font-medium text-gray-200 truncate pr-4">
                  ...
                  <div className="flex items-center gap-3 flex-shrink-0">
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded uppercase ${isSuccess ? 'bg-green-900/30 text-green-400 border border-green-500/20' : 'bg-red-900/30 text-red-400 border border-red-500/20'}`}>
                      {isSuccess ? 'Success' : 'Failed'}
                    </span>
                    {expandedCommands[index] ? <ChevronDown size={16} className="text-gray-500" /> : <ChevronRight size={16} className="text-gray-500" />}
                  </div>
                </button>

                {expandedCommands[index] && (
                  <div className="bg-[#0B0C10] p-4 border-t border-gray-700">
                    ...
                    {cmd.output && (
                      <div className="mt-3 pt-3 border-t border-gray-800">
                        <div className="text-[10px] text-gray-500 uppercase mb-2 font-bold">Standard Output</div>
                        <pre className="text-gray-400 text-xs font-mono overflow-x-auto max-h-40 custom-scrollbar">{cmd.output}</pre>
                      </div>
                    )}
                  </div>
                )}
          ...
          <div className="text-center py-12 bg-gray-800/50 rounded-lg border-2 border-dashed border-gray-700">
            ...
            <h4 className="text-gray-400 font-medium">No Logs Available</h4>
