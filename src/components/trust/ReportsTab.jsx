        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 text-xs mt-1 uppercase tracking-wider font-mono">
              {reportTypes.length} compliance report{reportTypes.length !== 1 ? 's' : ''} &bull; {assessmentItems.length} assessment document{assessmentItems.length !== 1 ? 's' : ''}
            </p>
          </div>
          {nextReportDate && (
      ...
      {reportManifest && (
        <div className={`${THEME.panel} rounded-xl border ${THEME.border} p-4`}>
          <div className="flex items-center gap-3 text-xs text-slate-500">
            <Clock size={12} />
            <span>Generated by <span className="text-slate-400 font-mono">{reportManifest.generator}</span></span>
            <span className="text-slate-700">|</span>
            <span className="font-mono">{new Date(reportManifest.generation_timestamp).toLocaleString()}</span>
            <span className="text-slate-700">|</span>
            <span>{reportManifest.provider?.service_name} — {reportManifest.provider?.impact_level}</span>
          </div>
        </div>
      )}
