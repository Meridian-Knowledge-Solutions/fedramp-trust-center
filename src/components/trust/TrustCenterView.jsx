...
export const TrustCenterView = () => {
    ...
    const downloadPackage = async () => { if (!guard('Download Certification Package')) return; try { const tok = localStorage.getItem(API_CONFIG.TOKEN_KEY); if (!tok) { alert('Session expired.'); return; } const r = await fetch(`${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.PACKAGE_DOWNLOAD}`, { headers: { Authorization: `Bearer ${tok}` } }); if (r.status === 401 || r.status === 403) { sessionExpired(); openModal('sessionExpired', { featureName: 'Download Certification Package' }); return; } if (!r.ok) throw new Error(`HTTP ${r.status}`); const j = await r.json(); if (j.url) window.location.href = j.url; } catch (e) { alert(`Download failed: ${e.message}`); } };
